...
const UserSettings = ({ username, themeMode, setThemeMode }) => {
  ...
  const handleProfilePictureChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        reader.readAsDataURL(file); // Converts to base64, which is stored as a string
      } catch (error) {
  ...
  const handleProfileSubmit = async () => {
    try {
      ...
      const profileDataToSave = {
        ...
        profilePicture: profileData.profilePicture || null, // Ensure null if no picture
        role: "user",
      };
    ...
    try {
      ...
      await setDoc(
        newUserRef,
        {
          ...
          theme: userData.theme || themeMode,
        },
        { merge: true }
      );
  ...
  return (
    <ThemeProvider theme={themeOptions[themeMode]}>
