...
const AgentSettings = ({ username, themeMode: propThemeMode, setThemeMode, onLogout }) => {
    // Use propThemeMode as initial value, with a fallback
            ...
            try {
                ...
                if (userSnap.exists()) {
                    ...
                    if (data.role === "agent") {
                        ...
                        // Sync theme from Firebase if available
                        if (data.theme && themeOptions[data.theme]) {
                            setLocalThemeMode(data.theme);
                            setThemeMode(data.theme); // Update parent themeMode
                        }
                    } else {
                        setErrorMessage("Access denied: User is not an agent.");
                    }
                } else {
                    setErrorMessage("Agent data not found.");
                }
            } catch (error) {
                setErrorMessage("Failed to load agent data: " + error.message);
            }
        };
        fetchAgentData();
        ...
        try {
            ...
            await setDoc(
                userRef,
                {
                    ...
                    passwordChange: false, // Set passwordChange to false
                    role: "agent",
                },
                { merge: true }
            );
    ...
    const handleThemeChange = (e) => {
        ...
        setThemeMode(newTheme); // Update parent themeMode
        const userRef = doc(db, "Users", username);
    ...
    const handleLogoutClick = () => {
        if (onLogout) {
            ...
            setLogoutDialogOpen(true); // Fallback if onLogout is not provided
        }
    };
    ...
    // Ensure a valid theme is always provided
            ...
            <Box sx={{ width: "100%", maxWidth: { xs: "100%", md: 600 }, mx: "auto", p: { xs: 2, sm: 3 } }}>
                ...
                <Card sx={{ mb: 2, borderRadius: 2, boxShadow: "0 2px 8px rgba(0, 0, 0, 0.1)", p: 2 }}>
                    ...
                    >
                        ...
                        {expandedSection === "profile" ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                    </CardContent>
